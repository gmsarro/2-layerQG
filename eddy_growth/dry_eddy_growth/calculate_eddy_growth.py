...
def compute_growth(
    *,
    ...
    u1: numpy.typing.NDArray[np.floating] = np.copy(u1_mean) * 0
    u2: numpy.typing.NDArray[np.floating] = np.copy(u2_mean) * 0
    ...
    peak_mode_structure_upper_img: numpy.typing.NDArray[np.floating] = np.copy(y) * 0
    peak_mode_structure_lower_img: numpy.typing.NDArray[np.floating] = np.copy(y) * 0
    peak_mode_structure_upper_real: numpy.typing.NDArray[np.floating] = np.copy(y) * 0
    peak_mode_structure_lower_real: numpy.typing.NDArray[np.floating] = np.copy(y) * 0
...
def main(
    *,
    ...
    logging.info('Starting eddy growth computation for %s', data_path)
...
def cli(
    *,
    data_path: Annotated[Path, typer.Option(help='Path to model NetCDF file')],
    output_file: Annotated[Path, typer.Option(help='Path to save NetCDF output')],
) -> None:
