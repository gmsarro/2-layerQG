...
## Moisture is solved using using the moist PV equation, precipitation happens if excess saturation appears (temperature dependent)
...
#    else:	print('converged at %i th iteration'%nIT)
    ...
    mean_P = np.mean(P)
    P -= mean_P
    excess_m = np.mean(m[rain_loc]-m_s[rain_loc])
    m[rain_loc] = m_s[rain_loc] #set m to the saturation after the excess precipitated out
    m[no_rain_loc] += excess_m #adjust m for the evaporation
